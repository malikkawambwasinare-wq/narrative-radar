...
- hook: the title a person sees on the shelf, in one of five shapes: a TALLY (a count that lands), a CLOCK (a date that moved or passed), a SPLIT (how the sides divide), a SCALE (the volume being pushed), or a CONTEST naming the competing explanation where the evidence proves nothing yet. NEVER a question: a question in a title plants the claim at about seventy per cent of the strength of asserting it. Lead with the FINDING, not a question: the specific thing this corpus can show, with its number or its year where there is one, in the narrative's own search terms, under 60 characters. "Anti-Inflammatory Diet: Nearly 1 in 3 Videos Is Selling Something" and "Housing Crash: A New Deadline Every Year Since 2021" are the register. A rhetorical question is the fallback when the evidence supports no finding yet, and never a coy one: "Is It Always Next Year?" reads as a sneer and says nothing. Never pair fear words with urgency words (warning, collapse, crash next to now, soon, about to), no ALL CAPS, no exclamation marks, no em dash. Examples of the register: "Housing Crash: Is It Always Next Year?", "Crypto Winter: Is the 4-Year Cycle Finally Dead?"
